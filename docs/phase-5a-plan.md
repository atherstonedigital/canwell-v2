...
| 8 | Email signup stub | Confirmed: visible button "Sign me up", on submit show non-functional inline confirmation ("Thanks. We'll send the first one soon."), no network call. Wire to an ESP in Phase 2. |
...
8. **ESP for the email form.** Brief defers form wiring to Phase 2, so 5A only stubs. Confirm the visible button reads "Sign me up" and on submit shows a non-functional confirmation, no network call.
